...
describe("editBounds", () => {
  ...
  it("returns first and last edit timestamp from committed events", () => {
    const out = editBounds(
      [
        ev({ type: "note_created", id: "1", ts: "2026-04-27T10:00:00Z", noteId: "n1", text: "a" }),
        ev({ type: "note_edited", id: "2", ts: "2026-04-27T10:00:05Z", noteId: "n1", text: "ab" }),
  ...
  it("blends in live input markers — earlier first, later last", () => {
    const out = editBounds(
      [
        ev({ type: "note_created", id: "1", ts: "2026-04-27T10:00:05Z", noteId: "n1", text: "a" }),
      ],
  ...
  it("does not regress bounds when live markers are inside the committed window", () => {
    const out = editBounds(
      [
        ev({ type: "note_created", id: "1", ts: "2026-04-27T10:00:00Z", noteId: "n1", text: "a" }),
        ev({ type: "note_edited", id: "2", ts: "2026-04-27T10:00:30Z", noteId: "n1", text: "abc" }),
      ],
      "2026-04-27T10:00:10Z",
